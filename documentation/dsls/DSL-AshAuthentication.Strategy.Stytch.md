...
| [`openid_configuration_uri`](#authentication-strategies-stytch-openid_configuration_uri){: #authentication-strategies-stytch-openid_configuration_uri } | `String.t` | `"/.well-known/openid-configuration"` | The URI for the OpenID provider Defaults to Stytch's OIDC discovery document at `/.well-known/openid-configuration`. Override only if you need the OAuth 2.1 authorization-server metadata or are pointing at a mock issuer. |
